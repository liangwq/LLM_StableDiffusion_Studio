...
class StableDiffusionText2ImageGenerator:
    ...
    def load_model(
        ...
        if self.pipe is None :
            self.pipe = StableDiffusionPipeline.from_pretrained(
                model_path, safety_checker=None, torch_dtype=torch.float16,cache_dir='./sd_model'
            )
        '''
        self.pipe = StableDiffusionPipeline.from_pretrained(
            model_path, safety_checker=None, torch_dtype=torch.float16,cache_dir='./sd_model'
        ...
        return self.pipe
    def image_model_path_change(self,
        model_path: str,
        prompt: str,
        negative_prompt: str,
    ...
    def app():
        #text2image_model_path='runwayml/stable-diffusion-v1-5'
        with gr.Blocks():
            state= gr.State()#,gr.State(text2image_model_path)
            with gr.Row():
                with gr.Column():
                ...
                #print(text2image_model_path)
                
            text2image_scheduler.select(
                fn=StableDiffusionText2ImageGenerator().image_model_path_change,
                inputs=[
