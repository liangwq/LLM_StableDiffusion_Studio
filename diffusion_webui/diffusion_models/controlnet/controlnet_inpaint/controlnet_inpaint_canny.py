...
class StableDiffusionControlNetInpaintCannyGenerator:
    ...
    def load_model(self, stable_model_path, controlnet_model_path, scheduler):
        if self.pipe is None:
            controlnet = ControlNetModel.from_pretrained(
                controlnet_model_path, torch_dtype=torch.float16,cache_dir='./control_model'
            )
            self.pipe = (
    ...
    def image_model_path_change(self,
        ...
        seed_generator: int,):
        self.pipe = None
        print(f'basemodel:{stable_model_path},cannymode:{controlnet_model_path}')
        normal_image = image_path["image"].convert("RGB").resize((512, 512))
        mask_image = image_path["mask"].convert("RGB").resize((512, 512))
    ...
    def app():
        with gr.Blocks():
            ...
            controlnet_canny_inpaint_model_id.select(
                fn=StableDiffusionControlNetInpaintCannyGenerator().image_model_path_change,
                inputs=[
            ...
            controlnet_canny_inpaint_stable_model_id.select(
                fn=StableDiffusionControlNetInpaintCannyGenerator().image_model_path_change,
                inputs=[
                    controlnet_canny_inpaint_image_file,
                    controlnet_canny_inpaint_stable_model_id,
